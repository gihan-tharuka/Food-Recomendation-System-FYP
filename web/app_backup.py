...
def rate_item():
    ...
    try:
        ...
        # Build explanation
        reason = []
        if row['cuisine'].lower() == preferences['cuisine'].lower():
            reason.append("✓ matches your preferred cuisine")
        ...
        if category in priority_weights:
            reason.append(f"✓ high priority category (weight={base_weight})")
        if row.get(time_col, False):
            reason.append(f"✓ suitable for {preferences['time_of_day']}")
        if row.get(weather_col, False):
            reason.append(f"✓ suitable for {preferences['weather']} weather")
        if rating:
            reason.append(f"✓ rated {rating}/5 by you")
    ...
    # Apply knapsack algorithm (simplified for demo)
    # Sort by value/price ratio and select within budget
    ...
    for item in items:
        if total_cost + item[0] <= preferences['budget']:
            selected_items.append(item)
            total_cost += item[0]
    
    # Format response
    recommendations = []
    for price, value, item_id, name, category in selected_items[:10]:  # Limit to 10 items
        recommendations.append({
            'id': item_id,
            'name': name,
            'price': price,
            'category': category,
            'explanation': explanations.get(item_id, 'No explanation available')
        })
...
def rate_items():
    ...
    try:
        data = request.json
        user_id = data.get('user_id')
        ratings = data.get('ratings', [])  # [{'item_id': 1, 'rating': 5}, ...]
        ...
        return jsonify({
            'success': True,
            'message': f'Saved {len(new_ratings)} ratings'
        })
